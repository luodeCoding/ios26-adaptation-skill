...
| 截止日期 | 要求内容 | 影响范围 | 阶段 |
|---------|---------|---------|------|
| **2026-04-28**（已生效） | 必须使用 iOS 26 SDK 构建 | 所有新提交和更新包 | **第一阶段** |
| **Xcode 27 发布前** (~2026-09) | Liquid Glass 完整适配，`UIDesignRequiresCompatibility` 失效 | 所有应用 | **第二阶段** |
| **用 iOS 27 SDK 构建起** (~2026-09) | UIScene 生命周期强制（未迁移 App 无法启动）、启动屏强制 | UIKit 应用 | **第三阶段（前瞻）** |
| **~2027-04（预估）** | 必须使用 iOS 27 SDK 构建 | 所有新提交和更新包 | **第三阶段** |

> 📅 完整时间线与各节点适配范围（唯一权威参考）：`docs/timeline.zh.md`；
> iOS 27 详细前瞻：`docs/ios27-preview.md`。
